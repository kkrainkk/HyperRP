...
class SafeRandomLoadText:
    ...
    def __call__(self, results: dict) -> dict:
        ...
        num_classes = len(class_texts)  # 动态获取类别数
        if 'gt_labels' in results:
            gt_label_tag = 'gt_labels'
        elif 'gt_bboxes_labels' in results:
            gt_label_tag = 'gt_bboxes_labels'
        ...
        # ⚠️ 新逻辑：负样本只从class_texts的合法类别里抽，避免越界
        num_neg_samples = min(
            min(num_classes, self.max_num_samples) - len(positive_labels),
            random.randint(0, num_classes - len(positive_labels)))
        ...
        # Debug日志，方便你验证
        print(f"[DEBUG] Positive labels: {positive_labels}")
        print(f"[DEBUG] Negative labels: {negative_labels}")
        print(f"[DEBUG] Sampled labels: {sampled_labels}")
